...
void on_node_info(void* object, const struct pw_node_info* info) {
  ...
  // Exclude output streams without an application identity (e.g. WirePlumber loopback streams).
  // These system-managed streams should not be intercepted by JamesDSP as it can cause
  // a feedback loop: jamesdsp_sink -> jdsp_filter -> loopback sink -> loopback stream -> jamesdsp_sink.
  // PW_KEY_APP_ID is not set in on_registry_global, so this check must happen here.

  if (!remove_node && nd->nd_info->media_class == tags::pipewire::media_class::output_stream) {
    if (spa_dict_lookup(info->props, PW_KEY_APP_ID) == nullptr) {
      remove_node = true;
    }
  }
